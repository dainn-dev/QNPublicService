// ============================================================
// Cổng cán bộ — Xác thực: màn đăng nhập + gate role officer.
// Bao ngoài OfficerApp: chỉ render cổng khi đã đăng nhập và có quyền
// (officer / admin / super). Phụ thuộc window.Auth (js/auth-api.js).
// ============================================================
...
// Ghi danh tính cán bộ thật vào window.ODATA.me (thay dữ liệu mock).
// officer có thể null (vd. admin/super không có OfficerProfile) → dùng /me.
function opApplyIdentity(user, officer) {
  const name = (officer && officer.fullName) || (user && user.name) || 'Cán bộ';
  const title = officer && (officer.position || officer.department);
  window.ODATA.me = {
    id: (officer && officer.profileId) || (user && user.id) || null,
    userId: user && user.id || null,
    name: name,
    role: { vi: title || 'Cán bộ', en: title || 'Officer' },
    initials: opInitials(name),
    roles: (user && user.roles) || [],
  };
}

// ---------- Màn đăng nhập ----------
function OfficerLogin({ lang, setLang, onSuccess }) {
  const t = useT(lang);
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (busy) return;
    if (!email.trim() || !password) { setError(t('op_login_required')); return; }
    setError('');
    setBusy(true);
    try {
      const user = await window.Auth.login(email, password);
      // Gate role: chỉ officer/admin/super mới được vào.
      if (!window.Auth.hasOfficerAccess(user)) {
        await window.Auth.logout();
        setError(t('op_login_err_forbidden'));
        setBusy(false);
        return;
      }
      // Xác định OfficerProfile của mình (best-effort — admin có thể không có).
      let officer = null;
      try { officer = await window.Auth.getCurrentOfficer(); } catch (err) { officer = null; }
      opApplyIdentity(user, officer);
      onSuccess(user);
    } catch (err) {
      const status = err && err.status;
      if (status === 400 || status === 401) setError(t('op_login_err_credentials'));
      else if (status === 403) setError(t('op_login_err_forbidden'));
      else setError(t('op_login_err_generic'));
      setBusy(false);
    }
  };
      ...
      <div className="fade-up" style={{ width: '100%', maxWidth: 408 }}>
        ...
        <div className="card" style={{ padding: '32px 30px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 24 }}>
            <Logo size={48} />
            <h1 style={{ fontSize: 'var(--fs-20)', marginTop: 14 }}>{t('op_login_title')}</h1>
            <p style={{ fontSize: 'var(--fs-14)', color: 'var(--ink-3)', marginTop: 6, lineHeight: 1.5 }}>{t('op_login_sub')}</p>
          </div>

          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span className="field-label">{t('op_login_email')}</span>
              <input className="input" type="email" autoComplete="username" autoFocus
                value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder={t('op_login_email_ph')} disabled={busy} />
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span className="field-label">{t('op_login_password')}</span>
              <input className="input" type="password" autoComplete="current-password"
                value={password} onChange={(e) => setPassword(e.target.value)}
                placeholder={t('op_login_password_ph')} disabled={busy} />
            </label>
            ...
            <button type="submit" className="btn btn-primary btn-lg" disabled={busy}
              ...
              {busy ? t('op_login_signing') : t('op_login_submit')}
            </button>
          </form>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 18 }}>
          <a href="Home.html" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 'var(--fs-13)', fontWeight: 600, color: 'var(--ink-3)' }}>
            <Icon name="arrowleft" size={15} />{t('op_login_back_citizen')}
          </a>
        </div>
      </div>
    </div>
...
function OfficerAuthSplash({ lang }) {
  const t = useT(lang);
  return (
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'var(--bg-soft)', color: 'var(--ink-3)', fontSize: 'var(--fs-15)', gap: 14 }}>
      <Logo size={44} />
      <span>{t('op_login_checking')}</span>
    </div>
  );
}

// ---------- Gate: checking → login → ready ----------
function OfficerRoot() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!window.Auth.hasOfficerAccess(user)) { window.Auth.clearSession(); if (active) setPhase('login'); return; }
        let officer = null;
        try { officer = await window.Auth.getCurrentOfficer(); } catch (e) { officer = null; }
        opApplyIdentity(user, officer);
  ...
  if (phase === 'checking') return <OfficerAuthSplash lang={lang} />;
  if (phase === 'login') return <OfficerLogin lang={lang} setLang={setLang} onSuccess={() => setPhase('ready')} />;
  return <OfficerApp onLogout={handleLogout} />;
}
